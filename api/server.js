  ...
  try {
    ...
    const stats = await pool.query(`
      SELECT
        ...
        (SELECT COUNT(DISTINCT owner_id) FROM boards) as active_users,
...
// 2. Список всех пользователей (с пагинацией и поиском)
app.get('/api/admin/users', {
  preHandler: [authenticateToken, checkAdmin]
}, async (req, reply) => {
  try {
    const { page = 1, limit = 20, search = '', plan = '', sortBy = 'created_at', sortOrder = 'DESC' } = req.query
    ...
    if (plan) {
      conditions.push(`sp.code_name = $${paramIndex}`)
      params.push(plan)
      paramIndex++
    }
    ...
    const users = await pool.query(`
      SELECT
        ...
        (SELECT COUNT(*) FROM boards WHERE owner_id = u.id) as boards_count,
        (SELECT COUNT(*) FROM active_sessions WHERE user_id = u.id AND expires_at > NOW()) as active_sessions
      FROM users u
      LEFT JOIN subscription_plans sp ON u.plan_id = sp.id
      ${whereClause}
...
// 3. Детальная информация о пользователе
  ...
  try {
    ...
    const user = await pool.query(`
      SELECT
        ...
        )) FROM active_sessions WHERE user_id = u.id) as sessions
      FROM users u
      LEFT JOIN subscription_plans sp ON u.plan_id = sp.id
      WHERE u.id = $1
    `, [userId])
...
// 4. Изменение плана пользователя
app.post('/api/admin/users/:userId/change-plan', {
  preHandler: [authenticateToken, checkAdmin]
}, async (req, reply) => {
  try {
    const { userId } = req.params
    ...
    try {
      ...
      newExpiresAt.setDate(newExpiresAt.getDate() + (durationDays || plan.rows[0].duration_days))
      ...
      await client.query(`
        ...
        WHERE id = $3
      `, [planId, newExpiresAt, userId])

      // Создаём запись в истории
      await client.query(`
        INSERT INTO subscription_history (
          user_id, plan_id, start_date, end_date,
          source, amount_paid, currency
        ) VALUES ($1, $2, NOW(), $3, 'admin_change', 0, 'RUB')
      `, [userId, planId, newExpiresAt])

      // Логируем
      await client.query(`
        INSERT INTO system_logs (event_type, severity, message, metadata)
        VALUES ('admin_plan_change', 'info', 'Администратор изменил план пользователя', $1)
      `, [JSON.stringify({
