...
### Уведомления при смене пароля

При успешной смене пароля пользователь получает уведомления:

#### Email-уведомление
Отправляется письмо с информацией о смене пароля (дата, время, геолокация).

#### Telegram-уведомление (если привязан)
- **Текст:** Информация о смене пароля с датой и геолокацией
- **Предупреждение:** Призыв восстановить доступ, если это были не вы
- **Inline-кнопка:** "👤 В профиль" → `https://interactive.marketingfohow.ru/`

---

### История изменений

#### 2026-01-30: Добавлена кнопка "В профиль" к Telegram-уведомлению о смене пароля
- Удалены лишние строки (ссылка на сайт, контакт админа) — информация дублировалась
- Добавлена inline-кнопка "👤 В профиль" для быстрого перехода в профиль
