...
│   ├── images/              # Библиотека изображений (модульная структура)
│   │   ├── index.js         # Агрегатор модулей (~25 строк)
│   │   ├── myLibrary.js     # Личная библиотека (~680 строк)
│   │   ├── shared.js        # Общая библиотека (~260 строк)
│   │   └── proxy.js         # Прокси для изображений (~130 строк)
...
### Библиотека изображений (`routes/images/`)

Модульная структура для работы с изображениями пользователей.

#### Личная библиотека (`images/myLibrary.js`)

| Метод | Endpoint | Описание |
|-------|----------|----------|
| GET | `/api/images/my/folders` | Список папок личной библиотеки |
| POST | `/api/images/my/folders` | Создать папку |
| GET | `/api/images/my/stats` | Статистика библиотеки |
| GET | `/api/images/my` | Список личных изображений |
| POST | `/api/images/upload` | Загрузить изображение |
| DELETE | `/api/images/:id` | Удалить изображение |
| PATCH | `/api/images/:id/rename` | Переименовать изображение |

#### Общая библиотека (`images/shared.js`)

| Метод | Endpoint | Описание |
|-------|----------|----------|
| POST | `/api/images/:id/share-request` | Отправить на модерацию |
| GET | `/api/images/shared` | Структура общей библиотеки |

#### Прокси (`images/proxy.js`)
