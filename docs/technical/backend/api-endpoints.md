...
- При первой верификации email генерируется `personal_id` в формате `{office} + 9 цифр` (например: `RUY000000000028`). Если у пользователя установлено представительство (`office`), оно используется как префикс. По умолчанию — `RUY000`. При активации также устанавливается `office = 'RUY000'`, если оно не было задано ранее.
