...
- При первой верификации email генерируется `personal_id` в формате `{office} + 9 цифр` (например: `RUY000000000026`). Если у пользователя установлено представительство (`office`), оно используется как префикс. По умолчанию — `RUY00`.
