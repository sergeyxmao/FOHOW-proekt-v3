...
Обрабатывает событие изменения PV (Product Volume) при клике на синий кружок-монетку в карточке лицензии.

```javascript
const handlePvChanged = (cardId) => {
  console.log('🔵 PV changed for card:', cardId);

  // Запускаем анимацию линий вверх по цепочке
...
**Поведение:**
- Анимирует соединительные линии вверх по дереву MLM-структуры
- Анимирует карточки вверх по цепочке (если включена настройка анимации)
- Создает визуальный эффект распространения PV от дочерних к родительским карточкам

## История изменений

### 2026-01-12: Исправление анимации при клике на синий кружок-монетку

**Проблема:** При клике на синий кружок-монетку в карточке лицензии анимировались только линии, но не карточки. При клике на кнопку "+10" работало корректно (анимировались и линии, и карточки).
...
- Добавлена проверка `isUserCardAnimationEnabled.value` для соблюдения настроек пользователя
- Теперь поведение при клике на монетку идентично клику на "+10"
