...
Инструмент "Текст" позволяет добавлять текстовые надписи на canvas с возможностью перемещения и масштабирования перед окончательной вставкой.

> Подробная документация: [docs/technical/text-tool.md](../../text-tool.md)
...
4. Ввести текст и нажать **Enter**
5. В режиме превью:
   - **Перетащить** текст в нужное место (за центр рамки)
   - **Изменить размер** за угловые маркеры (пропорционально, 0.5x - 3x)
6. **Enter** или клик вне рамки — текст рисуется на canvas
7. **Escape** — отмена на любом этапе

### Режимы работы

```
idle → typing → preview → commit
  ↑       ↓        ↓
  └── Escape ──────┘
```
...
const textSize = ref(24);          // Базовый размер шрифта
```
...
- `startDragText(event)` — начать перетаскивание
...
- `.pencil-overlay__text-preview` — контейнер превью
- `.pencil-overlay__resize-handle--*` — угловые маркеры масштабирования

## История изменений

### 2026-02-02: Режим превью для инструмента "Текст"
...
- Улучшенный дизайн рамок (яркая синяя рамка 2px, белый фон, тень)
