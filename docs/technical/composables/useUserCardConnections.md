...
**Назначение:** Поиск следующей родительской UserCard в иерархии (движение вверх по цепочке).

**Параметры:**
- `userCardId` - ID текущей user_card
- `visited` - Set посещенных карточек (для предотвращения циклов)

**Возвращает:**
- `{ connectionId, nextUserCardId }` - найденное соединение и ID родителя
- `null` - если родитель не найден
...
UserCard A (точка 3) → UserCard B (точка 1): найдёт B как родителя A ✅
