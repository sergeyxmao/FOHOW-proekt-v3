...
- **Форматы экспорта** зависят от тарифа (`can_export_png_formats` в features):
  - Guest — только A4 (`["A4"]`)
  - Individual/Premium — A4 + A3 (`["A4", "A3"]`)
  - Админ — все форматы, независимо от тарифа
- Если формат по умолчанию (A3) недоступен — автоматически выбирается первый доступный
...
| Параметр | Значение |
|----------|----------|
| Формат | Зависит от тарифа: Guest — A4, остальные — A4/A3, админ — все |
...
const pageFormats = computed(...)       // фильтруется по can_export_png_formats из тарифа
const selectedFormat = ref(...)        // 'a3' если доступен, иначе первый доступный
...
### 2026-02-21
- Форматы экспорта (A4/A3) теперь фильтруются по тарифу пользователя (`can_export_png_formats`)
- Guest видит только A4, платные тарифы — A4 и A3, админ — все форматы
- Если формат по умолчанию (A3) недоступен — автоматически выбирается первый доступный
- `pageFormats` стал `computed` вместо статического массива
