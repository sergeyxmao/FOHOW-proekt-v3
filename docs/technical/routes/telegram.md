...
При успешной привязке Telegram к аккаунту пользователь получает сообщение с:
- Подтверждением привязки
- Email привязанного аккаунта
- Списком уведомлений, которые будет получать
