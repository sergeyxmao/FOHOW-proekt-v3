...
### 2026-01-17 (v3): Исправление чтения transform после перемещения холста

**Проблема:** После перемещения холста (pan) функция `getViewportAnchoredPosition` могла читать устаревшие координаты transform.

**Причина:** Использовался fallback `content.style.transform || getComputedStyle()`, что могло давать некорректные значения.

**Исправление:**
- Добавлен принудительный reflow через `container.getBoundingClientRect()` перед чтением координат
- Изменено чтение transform: теперь всегда используется `window.getComputedStyle(content).transform`
- Убран fallback на `content.style.transform` для гарантии актуальности данных

**Результат:**
- Координаты viewport всегда актуальны после любых изменений pan/zoom
- Шаблоны вставляются в корректную позицию независимо от перемещений холста
