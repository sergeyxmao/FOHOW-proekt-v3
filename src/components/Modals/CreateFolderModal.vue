...
const createFolder = async () => {
  ...
  const duplicate = boardFoldersStore.folders.find(
    f => f.name.toLowerCase() === folderName.value.trim().toLowerCase()
  )
  ...
  try {
    await boardFoldersStore.createFolder(folderName.value.trim())
    emit('created')
  } catch (err) {
    if (err.response?.status === 403) {
      error.value = 'Достигнут лимит папок для вашего тарифа'
    } else if (err.response?.status === 409) {
      error.value = 'Папка с таким именем уже существует'
    } else {
      error.value = 'Не удалось создать папку. Попробуйте позже'
    }
  } finally {
    loading.value = false
  }
