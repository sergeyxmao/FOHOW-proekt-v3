...
export const useStickersStore = defineStore('stickers', () => {
  ...
  async function addSticker(boardId, stickerData) {
    ...
    try {
      ...
      if (!response.ok) {
        // Проверяем, не превышен ли лимит или подписка
        if (response.status === 403) {
          try {
            ...
            if (errorData.code === 'SUBSCRIPTION_EXPIRED') {
              alert(errorData.error || 'Срок вашей подписки истек');
              isLoading.value = false;
              return null;
            }
            
            // Если другая ошибка 403
            alert(errorData.error || 'Доступ запрещен');
            isLoading.value = false;
            return null;
            
          } catch (parseError) {
            // Если не удалось распарсить JSON
            alert('Ошибка доступа. Проверьте подписку.');
            isLoading.value = false;
            return null;
          }
        }
        
        // Для других статусов (не 403)
        try {
          const data = await response.json();
          throw new Error(data.error || 'Ошибка создания стикера');
        } catch (parseError) {
          throw new Error('Ошибка создания стикера');
        }
      }
      const data = await response.json();
