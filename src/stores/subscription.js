  ...
  actions: {
    ...
    async loadPlan() {
      ...
      try {
        ...
        // Проверяем, истекла ли подписка с учётом grace-периода
        const subscriptionExpiresAt = data.user?.subscriptionExpiresAt
        const gracePeriodUntil = data.user?.gracePeriodUntil
        const now = new Date()
        let isExpired = false

        if (subscriptionExpiresAt) {
          const expireDate = new Date(subscriptionExpiresAt)
          if (now > expireDate) {
            // Подписка истекла — проверяем grace-период
            if (!gracePeriodUntil || now > new Date(gracePeriodUntil)) {
              isExpired = true
            }
          }
        }

        // Если подписка истекла и grace-период тоже — показываем Guest-тариф
        if (isExpired) {
          this.currentPlan = {
            id: data.plan?.id,
            name: 'Guest (подписка истекла)',
            code_name: 'guest',
            priceMonthly: 0,
            expiresAt: subscriptionExpiresAt
          }
          // Устанавливаем лимиты Guest-тарифа
          this.features = {
            max_boards: 1,
            max_stickers: 10,
            max_notes: 10,
        ...
        this.limits = {
          boards: this.features?.max_boards ?? -1,
          notes: this.features?.max_notes ?? -1,
          stickers: this.features?.max_stickers ?? -1,
          comments: this.features?.max_comments ?? -1,
          cards: this.features?.max_licenses ?? -1
        }
    ...
    checkLimit(resourceType) {
      // Маппинг: comments → userComments, cards → userLicenses (для совместимости с API)
      const usageKey = 
        resourceType === 'comments' ? 'userComments' :
        resourceType === 'cards' ? 'userLicenses' :
        resourceType
